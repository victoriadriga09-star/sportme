...
export function GoogleButton({ label = "Continuer avec Google", onAfter }: { label?: string; onAfter?: () => void }) {
  return (
    <div className="space-y-3">
      <button
        type="button"
        onClick={() => {
          toast("Google sign-in bientôt disponible", { description: "On continue en mode démo." });
          onAfter?.();
        }}
        className="w-full h-12 rounded-2xl bg-surface border border-border text-ink font-semibold flex items-center justify-center gap-3 active:scale-[0.99] transition soft-shadow"
      >
