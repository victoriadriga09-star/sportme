    ...
    <main className="min-h-[100dvh] pb-32 bg-background">
      ...
      <div className="px-5 space-y-5">
        {SECTIONS.map((s) => (
          <section key={s.title}>
            ...
            <div className="rounded-3xl bg-surface border border-border divide-y divide-border overflow-hidden">
              {s.items.map((it: any) => (
                <button key={it.label} className="flex items-center gap-3 px-5 py-3.5 w-full text-left active:bg-muted">
                  <span className="flex-1 text-sm font-medium">{it.label}</span>
                  {"toggle" in it ? (
                    <Toggle on={!!toggles[it.label]} onChange={(v) => setToggles((p) => ({ ...p, [it.label]: v }))} />
                  ) : (
                    <>
        ...
        <section>
          ...
          <div className="rounded-3xl bg-surface border border-border divide-y divide-border overflow-hidden">
            <button className="flex items-center gap-3 px-5 py-3.5 w-full text-left text-sm font-medium">Se déconnecter</button>
            <button className="flex items-center gap-3 px-5 py-3.5 w-full text-left text-sm font-semibold text-destructive">Supprimer mon compte</button>
          </div>
        </section>
      </div>
    </main>
